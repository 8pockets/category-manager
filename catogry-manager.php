...
function category_manager_content_filter($args, $taxonomies) {
	...
	if(is_single('13') && 'category' !== $taxonomies[0]){
		return $args;
	}
	//page指定でカテゴリーの非表示を止める
	if(is_page('17') && 'category' !== $taxonomies[0]){
		return $args;
	}
...
{
	$myJsFile = TOURL . '/js/jquery-2.1.1.min.js';
	wp_register_script('jquery-2.1.1.min.js', $myJsFile);
...
//print_r(array_values($temrs));
	...
	echo '<p><input type="checkbox" name="category[]" value="display-none" class="display-none" checked><input type="submit" value="更新" /></p></form></div>';
}